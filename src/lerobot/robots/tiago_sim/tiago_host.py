...
def send_msg(sock, msg):
    ...
    # Use protocol=4 for Python 3 (but use 2 if you need Python 2 compatibility)
    payload = pickle.dumps(msg, protocol=2)  # still using protocol=2 for compatibility
            ...
            try:
                ...
                try:
                    while not rospy.is_shutdown():
                        ...
                        # In Python 3, we need to handle bytes vs str differently
                        ...
                        if isinstance(message, str) and message == 'calibrate':
                            ...
                            rospy.logwarn(f"Received unknown message format from client: {type(message)}")
